...
APP_MAPPINGS = {
    ...
    "vlc": "C:\\Program Files (x86)\\VLC\\vlc.exe",
    "command prompt": "cmd",
    "control panel": "control",
    "settings": "start ms-settings:",
    "paint": "mspaint",
    "vs code": "C:\\Users\\Administrator\\AppData\\Local\\Programs\\Microsoft VS Code\\Code.exe",
    "postman": "C:\\Users\\gaura\\AppData\\Local\\Postman\\Postman.exe",
    "microsoft edge": "C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe",
    "whatsapp": "C:\\Users\\Administrator\\AppData\\Local\\WhatsApp\\WhatsApp.exe"
}
...
async def open_folder(path):
    try:
        ...
        logger.error(f"❌فائل open کرنے میں error آیا۔ {e}")
...
async def create_folder(path):
    try:
        ...
        return f"✅ Folder create ہو گیا: {path}"
    except Exception as e:
        return f"❌ فائل create کرنے میں error آیا۔: {e}"

async def rename_item(old_path, new_path):
    try:
        os.rename(old_path, new_path)
        return f"✅ نام بدل کر {new_path} کر دیا گیا۔"
    except Exception as e:
        return f"❌ نام بدلنا fail ہو گیا: {e}"
...
async def open(app_title: str) -> str:
    ...
    try:
        ...
        if focused:
            return f"🚀 App launch ہوا اور focus میں ہے: {app_title}."
        else:
            return f"🚀 {app_title} Launch کیا گیا, لیکن window پر focus نہیں ہو پایا۔"
    except Exception as e:
        return f"❌ {app_title} Launch نہیں ہوا۔: {e}"
...
async def close(window_title: str) -> str:
    ...
    return f"❌ Window بند ہو گیا ہے۔: {window_title}"
...
async def folder_file(command: str) -> str:
    ...
    if "rename" in command_lower:
        ...
        return "❌ rename command valid نہیں ہے۔"
    ...
    if "delete" in command_lower:
        ...
        return "❌ Delete کرنے کے لیے item نہیں ملے۔"
